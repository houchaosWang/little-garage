...
import { BOX_DAYS, addDays, onPromoted, dueReviews, onReviewResult } from '../js/mastery.js';
...
test('干净抽查逐盒升级，两次≥7天通过转巩固', () => {
  ...
  let r = onReviewResult(s, 1, true, today);      // box0 interval1 → box1, due +3
  ...
  onReviewResult(s, 1, true, today);              // box1 interval3 → box2, due +7
  assert.equal(s.mastery['1'].box, 2);
  today = s.mastery['1'].due;
  onReviewResult(s, 1, true, today);              // interval7 → passes7=1, box3, due +21
  assert.equal(s.mastery['1'].passes7, 1);
  assert.equal(s.mastery['1'].state, 'provisional');
  today = s.mastery['1'].due;
  r = onReviewResult(s, 1, true, today);          // interval21 → passes7=2 → solid, box4
  assert.equal(s.mastery['1'].state, 'solid');
  assert.equal(r.solid, true);
  assert.equal(s.mastery['1'].box, 4);
  today = s.mastery['1'].due;
  onReviewResult(s, 1, true, today);              // box stays capped at 4 (45d maintenance)
  assert.equal(s.mastery['1'].box, 4);
...
test('抽查失败：回炉+等级回滚+盒清零，且不再进入待抽查', () => {
  ...
  s.streak = 1;
  onPromoted(s, 1, '2026-09-01');
  const r = onReviewResult(s, 1, false, '2026-09-02');
...
test('回炉后重新晋升恢复待抽查', () => {
  const s = mkSkill(3);
  onPromoted(s, 1, '2026-09-01');
  onReviewResult(s, 1, false, '2026-09-02');
  s.level = 2;
